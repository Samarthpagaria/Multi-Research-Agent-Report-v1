...
const ColorOrb: React.FC<OrbProps> = ({
  ...
  const fallbackTones = {
    base: "oklch(95% 0.02 20)",
    accent1: "oklch(75% 0.15 25)",
    accent2: "oklch(80% 0.12 45)",
    accent3: "oklch(78% 0.14 15)",
  }
...
export function MorphPanel({
  ...
  const [internalShowForm, setInternalShowForm] = React.useState(false)
  ...
  const handleSuccess = React.useCallback((message: string) => {
    ...
    onSubmit?.(message)
  }, [triggerClose, onSubmit, isPinned])
  ...
  }, [showForm, triggerClose, isPinned])
  ...
  return (
    <div
      ...
      style={isPinned ? { width: "100%", height: "auto" } : { width: FORM_WIDTH, height: FORM_HEIGHT }}
    >
      <motion.div
        ref={wrapperRef}
        data-panel
        className={cx(
          "bg-background z-3 flex flex-col items-center overflow-hidden border",
          isPinned ? "relative" : "relative bottom-8 max-sm:bottom-5"
        )}
        initial={false}
        animate={{
